...
function basestation_do_post_author() {
  printf( __( '<span class="byline"><i class="meta-icon icon-user"></i> <span class="author"><a class="url fn n" href="%1$s" title="%2$s" rel="author">%3$s</a></span></span>', 'basestation' ),
  ...
  <?php if ( $featured_query->have_posts() ) {
    echo "\t<div class=\"row\">";
      echo "\t<div class=\"large-12 columns\">";
        echo "\t<ul data-orbit id=\"featured-carousel\" class=\"carousel slide\">"; ?>
        ...
        echo "\t</ul><!-- #featured-carousel -->";
      echo "\t</div><!-- .large-12 columns -->";
    echo "\t</div><!-- .row -->"; ?>
    <!--  <script type="text/javascript">
      ...
      // include orbit init here instead of this? -->
      <?php } // if(have_posts()) ?>
...
<header class="page-header">
  ...
  if ( is_category() ) {
    ...
    printf( __( 'Author Archives: %s', 'basestation' ), '<span><a class="url fn n" href="' . get_author_posts_url( get_the_author_meta( "ID" ) ) . '" title="' . esc_attr( get_the_author() ) . '" rel="me">' . get_the_author() . '</a></span>' );
...
/**
 * Displays a simple side nav with child pages of the current or page above.
 *
 * From Required Foundation - https://github.com/wearerequired/required-foundation
 *
 ...
 *
 */
function basestation_do_side_nav( $nav_args = '' ) {
  ...
  $nav_args = apply_filters( 'basestation_side_nav_args', wp_parse_args( $nav_args, $defaults ) );
  ...
  if ( $post->post_parent ) {
    ...
    if ( $nav_args['show_home'] == true ) {
      $nav_args['before'] .= '<li><a href="' . get_home_url() . '">' . __( '&larr; Home', 'basestation' ) . '</a></li><li class="divider"></li>';
    }
...
add_action( 'basestation_side_nav', 'basestation_do_side_nav' );